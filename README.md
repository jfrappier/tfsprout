...
tfsprout runs 86 checks over a Terraform Provider's Go source and reports patterns that cause bugs, fail provider schema validation at runtime, or diverge from Terraform Plugin SDK conventions — missing `CheckDestroy` in acceptance tests, contradictory schema fields, unstable resource IDs, hand-rolled validators that duplicate `helper/validation`.
...
86 active checks in four categories. The full list, with a page per check, is in the [check index](docs/reference/checks.md) — or browse them at <https://jfrappier.github.io/tfsprout/reference/checks/>.
...
| [`S`](docs/reference/checks.md#schema-checks) | Schemas — `Schema` definitions and attribute maps | 38 |
