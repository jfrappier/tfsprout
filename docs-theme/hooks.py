...
def render_check_page(check: dict, readme: str) -> str:
    ...
    # Alerts are converted before anything is indented into a tab: the
    # pattern is anchored to the start of a line and would stop matching once
    # the block carries a four-space tab indent.
    ...
    # The flagged/passing pair is a direct A/B comparison, and each block runs
    # to twenty-odd lines. Tabs keep both one click apart instead of pushing
    # the passing example below the fold.
    ...
    for title, content in sections:
        low = title.lower()
        if low == "flagged code":
            parts += ["## Examples", "", '=== "Flagged"', ""]
            parts += [textwrap.indent(content.strip(), "    "), ""]
            if passing is not None:
                parts += ['=== "Passing"', "", textwrap.indent(passing.strip(), "    "), ""]
        elif low == "passing code":
            continue  # emitted alongside the flagged tab above
        elif low == "ignoring reports":
            parts += ["## Ignoring reports", "", content.strip(), ""]
        elif low == "options":
            parts += ["## Options", "", content.strip(), ""]
        else:
            parts += [f"## {title}", "", content.strip(), ""]

    if flagged is None and passing is not None:
        parts += ["## Examples", "", passing.strip(), ""]

    parts += [
        "---",
        "",
        f'*This page is generated from [`{check["source"]}/README.md`]'
        f'({REPO_BLOB}/{check["source"]}/README.md), which lives beside the analyzer.*',
    ]
